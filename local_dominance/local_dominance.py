...
class LocalDominance:
    ...
    def initGui(self):
        ...
        icon_path = ':/plugins/local_dominance/icon.png'
        self.add_action(
            icon_path,
    ...
    def select_output_file(self):
        ...
        filename, __ = QFileDialog.getSaveFileName(self.dlg, "Output file ", os.path.dirname(currentInFile), '*.tif')
    ...
    def run(self):
        ...
        self.layers = [i for i in QgsProject.instance().mapLayers().values() if i.source()[-3:].lower() == 'tif' or i.source()[-4:].lower() == 'tiff']
        layer_list = []
        ...
        if result:
            ...
            with open(os.path.join(self.plugin_dir, 'argFile.txt'), 'w') as f_out:
                for i in argList:
                    f_out.write(i)
                    f_out.write('\n')
            call(['pythonw3.exe', os.path.join(self.plugin_dir, 'local_dominance_parallel.py')], shell=False)
